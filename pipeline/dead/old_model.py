...
def run_model_5():
    ...
    image_ds = tf.data.Dataset.list_files('data/satellite_tiles2/train/images/*.jpg', shuffle=False)
    mask_ds = tf.data.Dataset.list_files('data/satellite_tiles2/train/masks/*.jpg', shuffle=False)
    train_ds = tf.data.Dataset.zip((image_ds, mask_ds))
    print(type(train_ds))

    def image_read(img, mask):        
        return (tf.image.resize(tf.io.decode_jpeg(tf.io.read_file(img)), (128, 128)), 
        tf.image.resize(tf.io.decode_jpeg(tf.io.read_file(mask)), (128, 128)))
        

    maps_ds = train_ds.map(
        lambda img, mask: (tf.io.decode_jpeg(tf.io.read_file(img)),
        tf.io.decode_jpeg(tf.io.read_file(mask)))
        )
    print(type(maps_ds))
    ...
    train_dataset = maps_ds.cache().shuffle(BUFFER_SIZE).batch(BATCH_SIZE).repeat()
    train_dataset = train_dataset.prefetch(buffer_size=1)

    model = compile_model()
    history = model.fit(
        train_dataset,
        steps_per_epoch=707/BATCH_SIZE,
        epochs=5)

    # return train_ds, maps_ds
